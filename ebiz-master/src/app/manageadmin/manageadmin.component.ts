...
export class ManageadminComponent implements OnInit, AfterViewInit {
  ...
  checkedCheckbox(types: string, source: any, checked: string, username_checked: string) {
    ...
    let item = this.dataSource.find((x : any) => x.username === username_checked);
    let itemAll = this.allEmployees.find((x : any) => x.username === username_checked);
  ...
  applyFilter() {
    ...
    if (filterValue === '') {
      ...
      this.dataSource = this.allEmployees.filter((x : any) => x.displayname.toLowerCase().includes(filterValueLower)
  ...
  _emplist(word : any, array : any) {
    return array.filter((x : any) => !x.username.toLowerCase().includes(word.toLowerCase()));
  }
  ...
  addEmplist() {
    ...
    if (!this.checkUserDuplicate()) {
      ...
      this.MailList.forEach((element : any)=> {
        this.dataSource.push({
      ...
      console.log(this.dataSource);
      this.alerts.toastr_success('Mark the user for insert.');
    }
    else {
      this.msgAlert = true;
    }
  ...
  checkUserDuplicate() {
    ...
    this.MailList.forEach((element : any) => {
      ...
      ret = (this.dataSource.filter((x : any) => x.username.toLowerCase().includes(element.username.toLowerCase()))).length > 0 ? true : false;
      console.log(this.dataSource.filter((x : any) => x.username.toLowerCase().includes(element.username.toLowerCase())))
    });
    return ret;
  }
  ...
  deleteEmpList(username: string) {
    ...
    this.alerts.swal_confirm_delete('').then((val) => {
      ...
      if (val.isConfirmed) {
        ...
        this.dataSource.forEach(function (item : any, index : any, object : any) {
        ...
        // });
        console.log(this.defaultData);
        this.alerts.toastr_success('Mark the user for deletion.');
  ...
  checkboxChange(checked: string, types : any) {
    ...
    const onSuccess = (data : any) => {
      ...
      if (data.after_trip.opt1 == "true") {
        ...
        if (data.after_add_user.length === 0) {
          ...
          let msg = "";
          let no = 1;
          data.after_add_user.forEach((dr : any) => {
            if (no != 1) { msg += '\n'; }
