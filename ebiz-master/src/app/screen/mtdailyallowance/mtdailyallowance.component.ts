...
@Component({
  ...
  templateUrl: './mtdailyallowance.html',
...
export class MtdailyallowanceComponent implements OnInit {
  ...
  onloadX1() {
    ...
    const onSuccess = (data: any): void => {
      ...
      this.arr_data['allowance_list'].forEach((e : any)=> {
        if (typeof e === "object") {
  ...
  onloadX2() {
    ...
    const onSuccess = (data : any): void => {
      ...
      this.arr_data['allowance_type'].forEach(function (e : any) {
        if (typeof e === "object") {
  ...
  openModalx2(template: TemplateRef<any>,typeBtn: any) {
    this.tp_clone = template;
    let config: object = {
      class: "modal-lg",
      animated: true,
  ...
  travel_category_function(values: any) {
    var re;
  ...
  workplace_type_country(values: any) {
    var re;
  ...
  workplace_thailand_name(id_name: any) {
    //this.thai_city = data.master_province ;
    var re;
    // var thai_arr : any = this.dtregion['master_province'];
    re = "country"
    try {
      // thai_arr = thai_arr.filter((word : any) => word.id == id_name);
      // re = thai_arr[0].name;
    } catch (err) { }
    return re;
  }

  workplace_type_country_name(val_type : any, id_name: any) {
    var re;
  ...
  add_overseas() {
    ...
    var id_max = 0;
    var sort_by_max = 0;
    this.arr_data.allowance_list.forEach(function (e : any) {
      if (parseInt(e.id) >= id_max) { id_max = parseInt(e.id); }
      if (parseInt(e.sort_by) >= sort_by_max) { sort_by_max = parseInt(e.sort_by); }
    })

    var check_khcode = this.arr_data.allowance_list.filter((word : any) => word.kh_code == this.Allwance_arr.khrate && word.workplace == this.Allwance_arr.plan) ;
  ...
  AddMaster_price(typeX: any,ov_lo: any) {
    debugger
    var check_values;
    var errorText = "กรุณากรอกข้อมูลให้ครบ";
    var overnight_type_val : any ;
    if(ov_lo == "overseas"){
      overnight_type_val = ""
    }else{
  ...
  Save_D() {
    ...
    this.arr_dataX1['allowance_type'].forEach(function (e : any) {
      if (typeof e === "object") {
  ...
  Save_D2() {
    ...
    this.arr_dataX1['allowance_list'] = this.arr_data.allowance_list;
    this.arr_dataX1['allowance_list'].forEach(function (e : any) {
      if (typeof e === "object") {
        if(e.status == true){e.status = '1'}else{ e.status = '0' }
        //try{e.status = this.bool_to_str(e.status);}catch(err){}
        //e.status = this.bool_to_str(e.status);
  ...
  ActiveEvent(id: any, typeText: any, textX: any, txbtn: any) {
  ...
  Allowance_editType(id: any ,typeXX: any ) {
    ...
    kh_type_up, workplace: string, workplace_type_country,
    ...
    if(typeXX == 'Overseas'){
      if(workplace_type_country == 0 || workplace_type_country == '0'){
        ...
        var PlansX = zone.filter((word : any) => word.id == workplace);
        this.zone_new = PlansX[0]
      }else{
        this.zone_location = county;
        var PlansX = county.filter((word : any) => word.id == workplace);
        this.zone_new = PlansX[0]
      }
    }else{

        this.zone_location = Thai;
        var PlansX = Thai.filter((word  : any)=> word.id == workplace);
        //this.zone_new = PlansX[0]
  ...
  County_chang(zone: any,id: any) {
    ...
    try {
      this.country_select
      var re = this.country_select.filter((word : any) => word.id == zone.id);
      //console.log();
  ...
  type_chang(zone : any,id: any) {

  }

  data_oversea(){
    var re = this.arr_data.allowance_list.filter((word : any) => word.travel_category == 'oversea');
  ...
  data_local(){
    var re = this.arr_data.allowance_list.filter((word : any)=> word.travel_category == 'local');
    re.sort(function(a: any, b : any){
      if(a.kh_code < b.kh_code) { return -1; }
      if(a.kh_code > b.kh_code) { return 1; }
      return 0;
