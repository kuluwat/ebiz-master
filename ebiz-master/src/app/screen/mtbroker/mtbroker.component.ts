...
export class MtbrokerComponent implements OnInit {
  ...
  onloadX() {
    ...
    const onSuccess = (data : any): void => {
      ...
      this.arr_mtbroker.sort((a: any, b: any) => parseFloat(a.id) - parseFloat(b.id));

      this.arr_mtbroker.forEach((e : any)=>{
  ...
  Chang_Status_dlete(id : any){
    this.swl.swal_confirm_delete('').then((val) => {
      //this.getBoolean(
      if (val.isConfirmed) {
        this.arr_mtbroker.forEach(function(e : any){
          if(e.id == id){
  ...
  CheckStatusSave(val : any,id: any,typeX: any){
    debugger;
    console.log(val);
  ...
  Chang_Status_update(id: any){
    this.swl.swal_confirm_changes('Do you want to changes Status').then((val) => {
      //this.getBoolean(
      debugger;
      if (val.isConfirmed) {
