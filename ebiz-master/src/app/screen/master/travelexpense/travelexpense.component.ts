...
import { formatDateTime, useAuth } from '../../../function/globalfunction.component';
...
export class TravelexpenseComponent implements OnInit {
  ...
  constructor(
      ...
      // ตรวจสอบค่าของ dependencies
      // console.log('Appmain:', this.Appmain);
      console.log('http:', this.http);
      console.log('ws:', this.ws);
      console.log('fileuploadservice:', this.fileuploadservice);
  ...
  get_index_by_emp(ds: any,  emp_id : any) {
    if (ds.length > 0) {
  ...
  async sendRemark() {
    ...
    //     }
    //   };
    //   bodyX.travelexpense_main.forEach((item : any) => {
    //     item.status_trip_cancelled = 'true';
    //     item.remark = remark;
    //     item.action_change = 'true';
    //   });
    //   bodyX.travelexpense_detail.forEach((item : any) => {
    //     item.action_change = 'true';
    //   });
    //   bodyX.emp_list.forEach((item : any) => {
  ...
  copyDetailAllEmp(emp_id : any) {
    ...
    //     //delete img
    //     this.model_all.img_list.forEach((e : any) => {
    //       if (bcheck_all) {
    //         if (e.emp_id != emp_id) {
    ...
    //       .filter((v : any) => v.emp_id != this.emp_id)
    //       .map(({ emp_id  } : any) => emp_id);
    //     console.log(userList, 'userList');
    //     Img_data = Img_data.filter((v : any) => v.emp_id == this.emp_id && v.action_type != 'delete');
    //     userList.forEach((emp_id_outer : any) => {
    //       if (bcheck_all) {
    //         //!! อาจจะ copy img ไปไม่ได้เนื่องจากถ้า ลบไฟล์ไป ไฟล์ของคนที่ copy มาอาจถูกลบไปด้วย
    //         if (Img_data.length > 0) {
    //           for (let item of Img_data) {
    //             let findexpense_type = this.model_all.travelexpense_detail.find(({ id } : any) => id === item.id_level_1);
    //             if (findexpense_type) {
    //               let { expense_type } = findexpense_type;
    //               let findIndex = this.model_all.travelexpense_detail.findIndex(
    //                 ({ emp_id, expense_type: expense_types } : any) =>
    ...
    //                   let set_maxid = this.model_all.img_list.map((i : any) => +i.id);
    ...
    //     this.model_all.travelexpense_main.forEach((e : any) => {
    //       if (bcheck_all) {
    //         if (e.emp_id != emp_id) {
    //           e.action_change = 'true';
    //           courseCheck = true;
    ...
    //     this.model_all.travelexpense_detail.forEach((e : any) => {
    //       //กรณีเลือก all
    //       if (bcheck_all) {
    //         if (e.emp_id != emp_id) {
    //           let el = Data1;
    ...
    //       let empIndex = emp_list.findIndex(({ emp_id } : any) => emp_id === emp_id_Selected);
  ...
  async update_userByDOC(value : any, select: MatSelect) {
  ...
  setActiveLink() {}
  check_img(detail_id : any) {
    let img_list = this.model_all.img_list;
    let data_new = img_list.filter(
      (word : any) => word.emp_id === this.emp_id && word.id_level_1 == detail_id && word.action_type !== 'delete'
    );
  ...
  uploadFile(ev : any, item_id : any) {
    ...
    if (hasImg) {
      this.model_all.img_list.forEach((el : any) => {
  ...
  downloadFile(filename : any, url : any) {
    // fs.saveAs(url, filename);
  }
  FileName(detail_id : any): string {
    let url = '';
    let img_list = this.model_all.img_list;
  ...
  downloadFileBY_Item(detail_id : any) {
    let url = '';
    let img_list = this.model_all.img_list;
    let data_new = img_list.find(
      (word : any) => word.emp_id === this.emp_id && word.id_level_1 == detail_id && word.action_type !== 'delete'
  ...
  UpdateCurrency(data : any) {
    ...
    data.travelexpense_detail.forEach((e : any) => {
  ...
  Onload() {
    ...
    this.ws.callWs(BodyX, this.action_stage.action_load).subscribe(
      ...
      () => {
        // this.Appmain.isLoading = false;
        this.model_all.dtm_expense_type.reduce((cur : any, pre : any) => {
  ...
  get UserDetail() {
    return this.model_all.emp_list.filter((item : any) => item.emp_id === this.emp_id);
  }
  updateFormat() {
    this.model_all.travelexpense_detail.forEach((el : any) => {
  ...
  //   this.model_all.img_list = this.model_all.img_list.sort(function (a : any, b : any) {
  //     if (bool) {
      ...
      if (true) {
        ...
        if (check_detail) {
          ...
          if (this.model_all.travelexpense_main.length > 0) {
            this.model_all.travelexpense_main.forEach((el: any) => {
              if (el.emp_id == this.emp_id) {
                el.action_change = 'true';
        ...
        if (this.isAccessCopy) {
          ...
          let dsempUpdate: any[] = this.model_all.travelexpense_detail
            .filter(({ action_change } : any) => action_change === 'true')
            .map(({ emp_id } : any) => emp_id);
  ...
  get statusUpdated(): boolean {
    ...
    const CheckData = travelexpense_detail.some(({ action_type } : any) => action_type === 'insert');
  ...
  get StatusSAP(): boolean {
    ...
    const CheckData = travelexpense_main
      .filter(({ emp_id: emp_idx, action_type } : any) => emp_id === emp_idx)
      .some(({ send_to_sap } : any) => send_to_sap === 'true');
  ...
  get StatusCancel(): boolean {
    ...
    const CheckData = emp_list
      .filter(({ emp_id: emp_idx } : any) => emp_id === emp_idx)
      .some(({ status_trip_cancelled } : any) => status_trip_cancelled === 'true' || status_trip_cancelled === null);
  ...
  UpdateExchangeRate(item : any, Rate_Type: string) {
    let dsExchangeRate = this.model_all.m_exchangerate;
    let exchangeRate_Value = 0;
    let dsexchangerow = [];
    var Maxdate_checked =  formatDateTime(new Date)?.toLocaleUpperCase

    //console.log(Maxdate_checked)
    if (dsExchangeRate.length > 0) {
      dsexchangerow = dsExchangeRate.filter(
        (res : any) => res.currency_id == Rate_Type && res.date_from == Maxdate_checked && res.date_to == Maxdate_checked
      );
      ...
      if (dsexchangerow.length > 0) {
        ...
        try {
          dsexchangerow = dsExchangeRate.filter((res : any) => res.currency_id == Rate_Type);
          /* item.grand_total = "0"; */
          item.exchange_rate = '';
          item.action_change = 'true';
          item.grand_total = this.Cal_Gran_Total(item.total, item.exchange_rate, item.currency);
  ...
  convertInt(Fidle: string, item: any, ev?: any, ds?: any) {
    try {
      ev = ev.toString();
    } catch (ex) {}
  ...
  Cal_Gran_Total(Total : any, Rate : any, currency_id : any) {
    var XRate;
    var XTotal;
    var newvalue;
    try {
    ...
    try {
      if (currency_id != null && currency_id != 'null') {
        this.arr_m_exchangerate.forEach((e : any) => {
  ...
  get_data_by_user(ds : any, emp_id : any) {
    var arr = [];
    ...
    if (Array.isArray(ds)) {
      if (ds.length > 0) {
        ...
        arr.forEach((e) => {
          ...
          dtm_expense_type.forEach((x : any) => {
