...
import { useAuth } from '../../../function/globalfunction.component';
...
export class AccommodationComponent implements OnInit {
  ...
  accommodation_detail : any = [];
  accommodation_booking : any = [];
  accommodation_detail_raw : any = [];
  accommodation_booking_raw : any = [];
  accommodation_booking_detail : any = [];
  img_list : any = [];
  booking_master : any = [];
  barStatus = [
    {
  ...
  CancelData() {
    this.alert.swal_confirm_changes('Do you want to cancel the document ?').then((val) => {
      if (val.isConfirmed) {
  ...
  get docStatus() {
    return (Status: number) => {
      ...
      if (this.emp_list.length > 0) {
        ...
        let dt : any = this.emp_list.find((item : any) => item.emp_id === emp_id);
        if (dt) {
          // alert(1);
  ...
  onloadX() {
    ...
    const onSuccess = (data : any): void => {
      ...
      this.accommodation_booking.forEach((e : any) => {
      ...
      this.accommodation_detail.forEach(function (e : any) {
        if (typeof e === 'object') {
          var d1, d2;
      ...
      if (this.isCancelled === true) {
        ...
        this.alert.swal_success('Successfully canceled');
      }
      setTimeout(() => {
        this.appMain.isLoading = false;
        this.Final = false;
 ...
 */
  ...
  update_userByDOCchang(values : any) {
    ...
    this.arrayX['emp_list'].forEach(function (e : any) {
      if (e.emp_id == empxid) {
  ...
  select_newdata() {
    ...
    this.accommodation_booking.forEach(function (e : any) {
  ...
  modelChanged(event : any) {
    if (event.length >= 3) {
      ...
      this.countries_data.forEach(function (e : any, resultx : any) {
        if (typeof e === 'object') {
          console.log(resultx);
          e['new_column'] = '';
        }
  ...
  firstselectionChanged() {
    var cityF = this.data_city.filter(function (c : any) {
      return c.country == 'UK';
    });
 ...
 */
    ...
    this.alert.toastr_success('Add Accommodation');
    this.before_ChangUser = true;
  }

  sortbyFX(emp_select : any) {
    var ds = this.accommodation_detail.sort(function (a : any, b : any) {
  ...
  saveaction() {
    ...
    var result_data = this.arrayX['accommodation_detail'].filter((word : any) => word.emp_id == this.emp_select);
    var result_booking = this.arrayX['accommodation_booking'].filter((word : any) => word.emp_id == this.emp_select);
    result_booking = result_booking.filter((word : any) => word.action_type != 'delete');
    ...
    if (SaveState) {
      this.alert.swal_confirm_changes('Do you want to save the document ?').then((val) => {
        if (val.isConfirmed) {
          this.accommodation_detail.forEach(function (e : any) {
            function datePlusday4(values : any) {
              if (values == '' || values == null) {
          ...
          this.arrayX['emp_list'].forEach((e : any) => {
          ...
          const onSuccess = (data : any): void => {
            ...
            this.alert.swal_success(data.after_trip.opt2.status);
  ...
  deleterow(id : any , emp_idd : any) {
    this.alert.swal_confirm_delete('').then((val) => {
      if (val.isConfirmed) {
        this.accommodation_detail.forEach(function (e : any) {
          if (e.id == id) {
        ...
        //this.emp_select ;
        var filter_delete = this.accommodation_detail.filter((word : any) => word.action_type != 'delete');
        filter_delete = filter_delete.filter((word : any) => word.emp_id == this.emp_select);
    ...
    this.accommodation_detail.forEach(function (e : any) {
      if (typeof e === 'object') {
        ...
        try {
          ...
          var montF = new DatePipe('en-US');
          var dx1 : any = montF.transform(xdate1, 'dd-MMM-yyyy');
          var dx2 : any= montF.transform(xdate2, 'dd-MMM-yyyy');
          e['DatePlusCin'] = dx1;
          e['DatePlusCout'] = dx2;
        } catch (err) {
          e['DatePlusCin'] = dx1;
  ...
  datePlusday(values : any, index : any, textname : any) {
    var montF = new DatePipe('en-US');
  ...
  openModalx(template: TemplateRef<any>, text : any, index : any) {
  ...
  openModalx2(template: TemplateRef<any>, text : any, index : any) {
    this.tp_clone = template;
    let config: object = {
      class: 'modal-md',
      animated: true,
  ...
  Edit_BookingTable(id : any, index : any) {
  ...
  disblebox_booking(index : any, status : any) {
    this.accommodation_booking[index].action_change = 'true';
    console.log(status);
    if (status == true) {
  ...
  data_retune() {
    var datrf = this.accommodation_detail.filter((word : any) => word.emp_id == this.emp_select);
    var re = datrf.filter((word : any) => word.action_type != 'delete');
    var ds = re.sort(function (a : any, b: any) {
  ...
  model_action_change_book(emp_id : any ) {
    //alert();
    this.accommodation_booking.forEach(function (e : any) {
      if (e.emp_id == emp_id) {
        e.action_change = true;
  ...
  copyDetailAllEmp(emp_id : any) {
    ...
    var Data1 = ds1.filter(function (el : any) {
      return el.emp_id == emp_id;
    });
    var Data2 = ds2.filter(function (el : any) {
      return el.emp_id == emp_id;
    });
  ...
  search_recommend(type : any, index : any) {
    this.accommodation_booking[index].action_change = 'true';
    if (type == 'search') {
      if (this.accommodation_booking[index].search == true) {
        this.accommodation_booking[index].recommend = false;
  ...
  copyDetailAllEmp2(emp_id : any) {
    this.alert.swal_confirm('Do you want to paste information?', '', 'question').then((val) => {
      if (val.isConfirmed == true) {
        this.before_ChangUser = true;
        ...
        this.accommodation_detail.forEach((e : any) => {
          if (bcheck_all) {
            if (e.emp_id != emp_id) {
              e.action_change = 'true';
              e.action_type = 'delete';
        ...
        var Img_data = this.img_list.filter((v : any) => v.action_type != 'delete');
        Img_data = Img_data.filter((v : any) => v.emp_id == emp_id);
        ...
        if (bcheck_all) {
          ...
          let empIndex = emp_list.findIndex((item : any) => item.emp_id === emp_id_Selected);
  ...
  New_row_byEmp(emp_id : any, dsCopy : any, Img_data : any) {
