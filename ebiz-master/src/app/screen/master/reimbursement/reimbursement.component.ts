...
export class ReimbursementComponent implements OnInit {
  ...
  exportexcel(action_status): void {
    //call API
    //this.OnExport();
    if (action_status == 'send_mail') {
      ...
      let haschange = this.model_all.reimbursement_detail.filter(
        (res) => res.reimbursement_date_type != '' && res.emp_id == this.emp_id
      );
      console.log(this.model_all.reimbursement_detail);
      if (haschange.length > 0) {
        let dt = haschange.map((res) => {
  ...
  //excel js
  async generateExcel(header_data, data_cell, action_status) {
    // Excel Title, Header, Data
    const title = 'Reimbursement Form';
    const header = header_data;

    const data = data_cell;
    var filter_last_row = data_cell.filter((e) => e[ 4 ] != '-');
    const last_row_total = filter_last_row.reduce((a, b) => a + +b[ 4 ], 0);
    var displayarr = this.model_all.emp_list.filter((e) => e.emp_id == this.list_emp);
    ...
    var row_total = worksheet.addRow([ '', '', '', 'Total', last_row_total ]);
    let LENGTH_ROW_TOTAL: any = 0;
    data[ 0 ].forEach((d, i) => {
      var _cells = row_total[ '_cells' ].length - 1;
      row_total.eachCell((cell, number) => {
        cell.alignment = {horizontal: 'center'};
        cell.font = {name: 'Browallia New', family: 4, size: 16, bold: true};
        if (number > 4) {
          cell.font = {name: 'Browallia New', family: 4, size: 16};
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      if (action_status == 'send_mail') {
        ...
        this.alerts.toastr_sucess('Download File');
      }
    });
  }
  ...
  get userDisplay(): string {
    ...
    let find = this.model_all.emp_list.find(({emp_id}) => emp_id === emp_id_current);
    if (find) {
  ...
  update_userByDOC(VLAUE, select) {
    ...
    this.model_all.emp_list.forEach((e) => {
      if (e.emp_id == VLAUE) {
        e.mail_status = 'true';
      } else {
        e.mail_status = 'false';
  ...
  get_index_exchange_rate(ds, emp_id, id) {
    if (ds.length > 0) {
  ...
  RateChang(val) {
    ...
    dsexchangerow = this.arr_m_exchangerate.filter((res) => res.currency_id == val);
  ...
  ChangeASOF(inputDate) {
    ...
    if (currencyS && inputDate.value) {
      ...
      dsexchangerow = this.arr_m_exchangerate.filter((res) => res.currency_id == currencyS && res.date_from.toLowerCase() === as_of_str.toLowerCase());
  ...
  add_ExchangeRate(asofstr, exchange_value) {
    var exchangeDs = this.model_all.m_exchangerate;
    var bcheck = exchangeDs.findIndex((res) => {
  ...
  Update_Exchange_rate(template) {
    ...
    this.modalRef.hide();
    this.alerts.toastr_sucess('Update Exchange Rate');
    return;
    var date_checked = new Date(this.convert_dateDMY(as_of_str, 'yyyy-MM-dd'));
    var date_check: boolean = false;
    if (exchangeDs.length > 0) {
  ...
  openModal(template: TemplateRef<any>, emp_id, row_id) {
  ...
  OpenModal_Mail(template: TemplateRef<any>) {
    ...
    if (this.model_all.mail_list.length > 0) {
      this.model_all.mail_list.forEach((res) => {
  ...
  OpenModal_Export(template: TemplateRef<any>) {
    ...
    return ; */
      ...
      let dt = haschange.map((res) => {
        if (res[ 'reimbursement_date_type' ] != '' && res[ 'reimbursement_date_type' ] != null) {
          return {
            Date: this.convert_dateDMY(res[ 'reimbursement_date_type' ], 'MM/dd/yyyy'),
            Particulars: res.details == '' ? '-' : res.details,
            Total: res.total.replace(/,/g, '') == '' ? '-' : res.total.replace(/,/g, ''),
            Rate: res.exchange_rate == '' ? '-' : res.exchange_rate,
            'Total (THB)': res.grand_total.replace(/,/g, '') == '' ? '-' : res.grand_total.replace(/,/g, ''),
  ...
  async Config_Email_Send() {
    ...
    if (bcheck_mailtoo) {
      ...
      var irow = this.model_all.mail_list.findIndex((m_val) => m_val.emp_id == this.emp_id);
      if (irow > -1) {
  ...
  Send_Email() {
    ...
    BodyX.reimbursement_main.forEach((e) => {
      if (e.emp_id == empid_x) {
  ...
  search_len_id(ds, emp_id, doc_id) {
    var dt = ds.filter((res) => this.emp_id == emp_id && doc_id == this.doc_id);
  ...
  Onload() {
    ...
    const onSuccess = (data) => {
      ...
      this.model_all.reimbursement_detail.forEach((e) => {
        e[ 'Remark_X' ] = e.remark;
      });
    ...
    this.ws.callWs(BodyX, this.action_stage.action_load).subscribe(
      ...
      () => {
        ...
        if (this.model_all.reimbursement_detail.length > 0) {
          try {
            this.model_all.reimbursement_detail.forEach((el, i, ds) => {
  ...
  check_user() {
    if (this.model_all.user_admin == true || true) {
      const sortBy = (a, b) => (+a.id) - (+b.id)
      const dt = this.model_all.reimbursement_detail.filter((res, i, ds) => {
  ...
  OnsaveSendEmail(btn_type, template?) {
    return new Promise((resolve, reject) => {
      if (btn_type == 'saved') {
        ...
        if (true) {
          ...
          const OnsaveSucecss = (data) => {
            ...
            if (data.after_trip.opt1 == 'true') {
              ...
              this.model_all.reimbursement_detail.forEach((el) => {
          ...
          if (this.model_all.reimbursement_main.length > 0) {
            this.model_all.reimbursement_main.forEach((el) => {
          ...
          bodyX.reimbursement_detail.forEach((e) => {
  ...
  convert_Int_data(Fidle: string, index?, ev?) {
  ...
  convertInt(Fidle: string, item?, ev?: any, input?: any) {
    try {
      ev = ev.toString();
    } catch (ex) { }
    var newvalue = ev.replace(/,/g, '');
  ...
  calTotal() {
    ...
    this.model_all.reimbursement_detail.forEach((v) => {
      let parse: number;
  ...
  new_row(id, doc_id) {
    ...
    this.alerts.toastr_sucess('Add Reimbursement');
  }
  del_row(seq, doc_id) {
    var dt = this.model_all.reimbursement_detail;
    var bcheck = false;
  ...
  get_data_img() {
    return this.model_all.img_list.filter((res) => res.emp_id == this.emp_id);
  }
  check_action(item) {
