...
export class FeedbackComponent implements OnInit {
  ...
  async update_userByDOC(VLAUE, select) {
    var check_detail = this.model_all.feedback_detail.some(
      (res) => res.action_change == 'true' && res.emp_id == this.emp_id
    );
  ...
  Onload() {
    ...
    const onSuccess = (data) => {
      ...
      console.log(data);
      data.feedback_topic_list.forEach((element) => {
        element['show'] = true;
      });
  ...
  setClass(evRefP, evRefSmall, evcontainer, ellength) {
  ...
  async Onload_status_cancle() {
    const { isConfirmed } = await this.alerts.swal_confrim_changes('Do you want to cancel the document ?');
    if (isConfirmed) {
      this.Appmain.isLoading = true;
      const onSuccess = (data) => {
        this.Appmain.isLoading = false;
  ...
  update_data(update_answer, updae_action) {
    return true;
  }
  def_data(data) {
    this.model_all = data;
  ...
  async Onsave(btn_type) {
    ...
    if (btn_type == 'saved') {
      if (true) {
        ...
        const OnsaveSucecss = (data) => {
          ...
          if (data.after_trip.opt1 == 'true') {
            ...
            try {
              var i = this.get_index_by_emp(data.emp_list, this.emp_id);
              if (this.model_all.emp_list[i].hasOwnProperty('show_button')) {
      ...
      //submit
      //action_save: "SaveFeedback",
      const actionSave = await this.alerts.swal_confrim_changes('Do you want to submit the document ?');
      if (actionSave.isConfirmed) {
        this.Appmain.isLoading = true;
        const OnsaveSucecss = (data) => {
          this.Appmain.isLoading = false;
  ...
  DataBYCARID(topic_id) {
    ...
    let dt = this.model_all.feedback_detail.filter(
      (res) => res.topic_id == topic_id && res.feedback_type_id == feedback_type_id_checked && res.emp_id == this.emp_id
    );
    return dt;
  }
  check_action(item) {
