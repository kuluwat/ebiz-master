...
export class AirticketComponent implements OnInit {
  ...
  get UserDetail() {
    return this.maindata_all.emp_list.filter((item : any) => item.emp_id === this.emp_id);
  }
  ...
  get docStatus() {
    return (Status: number) => {
      ...
      if (this.maindata_all.emp_list.length > 0) {
        ...
        let dt : any = this.maindata_all.emp_list.find((item : any) => item.emp_id === emp_id);
        if (dt) {
  ...
  get fileByUser(): boolean {
    ...
    let callback1 = ({fullname, path, filename} : any) => fullname && path && filename;
    let callback2 = ({action_type, emp_id} : any) => action_type !== 'delete' && emp_id === emp_id_selected;
  ...
  onloadOld_data() {
    ...
    const onSuccess = (data : any) => {
      ...
      this.air_ticket_body.forEach(function (e : any) {
        if (typeof e === 'object') {
          var d1, d2;
    ...
    this.ws.callWs(bodyX, 'LoadAirTicket').subscribe(
      ...
      () => {
        ...
        if (typeof this.air_ticket_body == 'object') {
          this.air_ticket_body.forEach((val : any) => {
            if (val[ 'airticket_date' ] != '' && val[ 'airticket_date' ] != null) {
              let datestr : any = this.convert_dateYMD(val[ 'airticket_date' ]);
          ...
          this.def_data_old.airticket_booking.forEach((el : any) => {
  ...
  convert_textbooking(id_booking : any) {
    var dt : any = this.maindata_all.m_book_status;
    var result: any;
    for (const res of dt) {
  ...
  update_userByDOC(VLAUE : any) {
    ...
    console.log(this.maindata_all.airticket_detail);
    console.log(this.air_ticket_body);
    this.air_ticket_emplist.forEach((e : any) => {
    ...
    this.userDetail = this.UserDetail[ 0 ];
    const emp_id  : any = this.emp_id;
    const doc_id = this.doc_id;
    this.Appmain.userSelected = emp_id;
    this.TrackingStatus = {...InitTrackStatus};
  }
  get_index_airticket(ds : any, emp_id : any, id? : any) {
    if (ds.length > 0) {
      var i = ds.findIndex((res : any) => {
  ...
  get_index_airticket_by_row(ds : any , emp_id : any, id? : any) {
    if (ds.length > 0) {
  ...
  convert_dateYMD(val : any) {
    var montF = new DatePipe('en-US');
    var objdate : any = montF.transform(val, 'yyyy-MM-dd');
    return objdate;
  }
  setformat(val : any, item : any) {
    // YYYY-MM-DD
    let datestr : any = new Date(this.convert_dateYMD(val));
  ...
  stage_check(type : any, emp_id : any, id : any) {
    this.Log(type);
    this.Log(emp_id);
  ...
  New_row_byEmp(emp_id : any, dsCopy : any, ImgList : any) {
    ...
    this.air_ticket_body.forEach((e : any) => {
      var d1, d2;
      if (e.Datefrom == '') {
        d1 = '';
      } else {
  ...
  copyDetailAllEmp(emp_id : any ) {
    ...
    var Data1 : any = [],
      Data2 : any;
    this.alerts.swal_confirm('Do you want to paste information?', '', 'question').then((val) => {
      if (val.isConfirmed == true) {
        var irow = 0;
        ...
        Data2 = ds2.filter(function (el : any) {
          return el.emp_id == emp_id;
        });
        let courseCheck = false;

        //delete img
        this.DOC_DETAIL.img_list.forEach((e : any ) => {
          if (bcheck_all) {
            if (e.emp_id != emp_id) {
            ...
            ds_person_selected.forEach((el : any) => {
        ...
        var Img_data = this.DOC_DETAIL.img_list.filter((v : any) => v.action_type != 'delete');
        Img_data = Img_data.filter((v : any) => v.emp_id == this.emp_id);
        ...
        this.booking_detail.forEach((e : any) => {
          //กรณีเลือก all
          if (bcheck_all) {
            if (e.emp_id != emp_id) {
              // add detail
        ...
        if (bcheck_all) {
          ...
          let empIndex = this.air_ticket_emplist.findIndex(({emp_id} : any) => emp_id === emp_id_Selected);
  ...
  AuthAdmin(data : any) {
    ...
    if (userSelected) {
      let findIndex = data.emp_list.findIndex(({emp_id} : any) => emp_id === userSelected);
      if (findIndex > -1) {
  ...
  fileterbookdetail() {
    ...
    this.Log(obj);
    this.select_row_airticket = dt.findIndex((v : any) => {
      return v.id == obj.row_id && v.emp_id == obj.emp_id;
    });
    this.selected_bookdetail = ds.findIndex((v : any) => {
      return v.emp_id == obj.emp_id;
    });

    return;
  }
  clearselect(name : any, i : any) {
    // console.log(name)
  ...
  def_data(data : any, action : any) {
    ...
    if (action === 0 || action === 1) {
      data.airticket_booking.forEach((el : any) => {
        el.search_air_ticket = this.convert_bool(el.search_air_ticket);
        el.ask_booking = this.convert_bool(el.ask_booking);
        el.as_company_recommend = this.convert_bool(el.as_company_recommend);
        el.already_booked = this.convert_bool(el.already_booked);
        ...
        if (finduser) {
          ...
          if ('user_request' in data && data.user_request === true) {
            ...
            if (userSelected) {
              ...
              let finduser = data.emp_list.find(({emp_id} : any) => emp_id === this.emp_id);
              this.topping_selected = finduser ? finduser : data.emp_list[ 0 ];
            } else {
              this.emp_id = data.emp_list[ 0 ].emp_id;
              this.Appmain.userSelected = data.emp_list[ 0 ].emp_id;
  ...
  get_data_img(ds : any) {
    ...
    var dt = ds.filter((v : any) => v.emp_id == this.emp_id);
    return dt;
  }
  Log(txt: any) {
    console.log(txt);
  ...
  s_condition(action_type : any, emp_id_ds : any, item? : any) {
  ...
  get_data() {
    ...
    let findemp = null;
    try {
      findemp = this.air_ticket_emplist.find(({emp_id} : any) => emp_id === this.emp_id);
    } catch (ex) { }
    if (dt.length == 0 || !findemp) {
      return [];
    }

    dt = this.air_ticket_body.filter((val : any) => {
  ...
  get_emp_cur(token? : any): string {
    return this.emp_id;
  }

  new_row(id : any, doc_id : any, type_check? : any) {
    var dt = this.air_ticket_body;
    if (dt.length == 0) {
      return;
    }
    ...
    if (type_check != false) {
      this.alerts.toastr_success('Add Airticket');
    }
  }
  ...
  displayop(action_type : any, type_button : any, index : any) {
    var dt = this.air_ticket_body;
    dt.filter((val : any) => {
  ...
  Onsave(btn_type : any) {
    //debugger
    this.Appmain.isLoading = true;
    const convert_dateYMD = function (val : any) {
      var montF = new DatePipe('en-US');
    ...
    if (btn_type == 'save' || btn_type == 'submit') {
      ...
      var result_data = bodyX.airticket_detail.filter((word : any) => word.emp_id == check_id);
      var result_booking: any[] = bodyX.airticket_booking.filter(
        (word : any ) => word.emp_id == check_id && word.action_type != 'delete'
      );

      result_data = result_data.filter((word : any) => word.action_type != 'delete');
      // "ask_booking": "false",
      // "search_air_ticket": "false",
      // "as_company_recommend": "false",
      ...
      if (SaveState) {
        this.alerts.swal_confirm_changes(ConfremSubmit).then((val) => {
          if (val.isConfirmed == true) {
            //debugger;
            ...
            bodyX.airticket_detail.forEach((e : any) => {
            ...
            //!! update MAILSTATUS
            this.air_ticket_emplist.forEach((item : any) => {
            ...
            bodyX.img_list = imgList.filter(({filename, path} : any) => filename !== '' && path);
  ...
  openModalx(template: TemplateRef<any>, modal_id : any) {
    var dataAir = this.air_ticket_body.filter((word : any) => word.id == modal_id);

    var drop1 = dataAir[ 0 ].airticket_route_from;
    var drop2 = dataAir[ 0 ].airticket_route_to;

    var xairport_new1 = this.TestAirnewMt.filter((word : any) => word.id == drop1);
    var xairport_new2 = this.TestAirnewMt.filter((word : any) => word.id == drop2);
  ...
  airport_new1 : any = [];
  airport_new2 : any = [];
  master_airport_new_arr : any = [];
  master_airport_new_arrXX : any = [];
  TestAirnewMt : any = [];
  TitleFromx = 'Oversea';
  loadMasterAir() {
    this.Appmain.isLoading = true;
  ...
  filter_transform(items : any, searchText : any) {
    searchText = searchText.toLocaleLowerCase();
    return items.filter((it : any) => {
      return (
  ...
  destroyedChanged(event : any, typee : any) {
    console.log(event);
  }
  ...
  add_route() {
    ...
    try {
      ...
      this.maindata_all.airticket_detail.forEach((e : any) => {
        if (e.id == eid) {
          e.action_change = 'true';
      ...
      this.alerts.toastr_success('Add Route Sucess');
      this.airport_new1 = [];
      this.airport_new2 = [];
      this.modalRef?.hide();
    } catch (err) { }
  ...
  show_rout(i_id : any, typeX : any) {
    try {
      ...
      re = this.TestAirnewMt.filter((word : any) => word.id == i_id);
  ...
  changModelXTime(id : any, val : any, typeX : any) {
  ...
  min_max_date(index : any, type_date : any, val_date : any) {
    var dt = this.air_ticket_body;
