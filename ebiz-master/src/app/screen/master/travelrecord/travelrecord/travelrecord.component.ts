...
export class TravelrecordComponent implements OnInit {
  ...
  model = {
    ...
    min_business_date_from: null,
    max_business_date_to: null,
    keyword: "",
    dataSource: []
  }
  ...
  loadMaster() {
    ...
    const onSuccess = (data) => {
      ...
      data.m_country.forEach(current => {
      ...
      data.m_province.forEach(current => {
      ...
      data.m_traveltype.forEach(current => {
          ...
          const onSuccess = (dao) => {
            ...
            if (dao.length === 0) {
              ...
              const profile = _ => {
                ...
                const empLogin = loopSucess.employee.list.filter(val => val._id === empId);
                ...
                if (empLogin.length > 0) {
                  const selectedData = this.model.section.list.filter(val => val._id === empLogin[0]['sections']);
    ...
    try {
      if (!this.ws.isEmpty(event.value)) {
        if (!this.ws.isEmpty(event.value.sections)) {
          const selectedData = this.model.section.list.filter(val => val._id.toLowerCase().includes(event.value.sections.toLowerCase()));
        ...
        // select ddl department
        const selectedDepartment = this.model.department.list.filter(val => val._id.toLowerCase().includes(event.value.department.toLowerCase()));
        this.model.department.value = selectedDepartment;

        // select ddl function
        const selectedFunction = this.model.function.list.filter(val => val._id.toLowerCase().includes(event.value.function.toLowerCase()));
  ...
  changeSection(event) {
    ...
    try {
      if (!this.ws.isEmpty(event.value)) {
        const selectedData = this.model.department.list.filter(val => val._id.toLowerCase().includes(event.value.department.toLowerCase()));
  ...
  changeDepartment(event, action) {
    console.log('changeDepartment');
    console.log(event);
    try {
      if (!this.ws.isEmpty(event.value)) {
        const selectedData = this.model.function.list.filter(val => val._id.toLowerCase().includes(event.value.function.toLowerCase()));
