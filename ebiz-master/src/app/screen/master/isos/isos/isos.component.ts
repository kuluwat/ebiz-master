  ...
  send_mail_to_insurance() {
    ...
    const onSuccess = (data) => {
      ...
      if (data.after_trip.opt1 == 'true') {
        ...
        this.oldStr = this.htmlContent;
        this.appMain.isLoading = false;
        this.alerts.swal_sucess('The information has been updated');
...
// brokerData == []
    if (brokerData == '' || brokerData == 'undefined' || brokerData == null ) {
      ...
      this.modalRef.hide();
      this.array_result.m_broker.forEach((e) => {
    ...
    debugger;
    this.emp_listx.forEach(function (e) {
      if (e.Check_status == true) {
        e.mail_status = 'true';
      } else {
        e.mail_status = 'false';
  ...
  get EmpallhasISOSRECORD(): boolean {
    ...
    if (emp_list.length > 0) {
      showmsg = emp_list.every(({type_send_to_broker}) => this.getBoolean(type_send_to_broker) === true);
    }
    return showmsg;
  }
  ...
  business_name(bussEvent) {
    var res;
    this.business_type_listX.forEach(function (e) {
      if (e.id == bussEvent) {
        res = e.name;
      } else {
        res = '-';
  ...
  test_swal() {
    ...
    this.alerts.swal_confrim('Test', 'Do you want to delete the data?', 'error').then((val) => {
      if (val.isConfirmed) {
        //alert(`You typed: ${val.isConfirmed}`);
        this.alerts.swal_sucess('');
      } else {
  ...
  test_Toastr() {
    this.alerts.toastr_sucess('save data sucess.');
  }
