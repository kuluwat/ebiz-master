...
export class TravelerhistoryComponent implements OnInit {
  ...
  fnEdit(edit : any) {
    if (edit === 1) {
      ...
      var ArrFilterX = $.grep(this.arrayX.traveler_emp, function (v : any) {
        return (v.emp_id === eId);
      });
    ...
    const onSuccess = (data : any) => {
      ...
      if (this.arrayX.traveler_emp.length > 0) {
        ...
        if (this.arrayX.user_admin === false) {
          ...
          let arrX = this.arrayX.arrTraveler.filter(function (item : any) {
