...
import { BANKS, Bank } from '../../home/ebizhome.component';
...
import { useAuth, CloneDeep } from '../../../function/globalfunction.component';
...
isCanceled = false;
  ...
  update_userByDOC(event : any, empid : any, values : any) {
  ...
  get empByDoc() {
    const Filteremp = (item : any) => item.emp_id === this.empid;
    const countrygroup = this.country_doc.filter(Filteremp).map((item : any) => item.country_id);
  ...
  onloadX() {
    ...
    const onSuccess = (data : any): void => {
      ...
      const Filteremp = (item : any) => item.emp_id === this.empid;
      const countrygroup = data.country_doc.filter(Filteremp).map((item : any) => item.country_id);
      this.country_group = [ ...countrygroup ];
      //!!  หา group ประเทศที่จะไป
      //debugger;
      ...
      function getMaxIn(arr : any, prop : any) {
        var max;
      ...
      function filter_nationality(valueX : any, typeC : any) {
        //debugger;
        var visa;
  ...
  async assingData() {
    ...
    const $LoadEmployeeList = (data : any) => {
    ...
    const $LoadMasterDataCountry = (data : any): void => {
      console.log('country');
      console.log(data);
      this.visa_docountries = data.visa_docountries;
      this.appMain.isLoading = false;
    };
    const $LoadMasterDataDOC = (data : any) => {
      this.masterDoc = data.visa_document;
    };
    const $LoadMasterDataLocation = (data : any): void => {
      this.zone_select = data.master_zone;
      this.country_select = data.master_country;
      this.county_new_arr = data.master_country;
      this.country_list = data.master_country;
  ...
  download_file_check(visa_id : any, empid : any) {
    if (this.CheckMtCounty != true) {
      function downloadFile_In_Function(url : any, filename : any) {
        let Regex = /.[A-Za-z]{3}$/;
  ...
  onFileSelect(event : any, id_visa : any, emp : any, type_name : any) {
    if (this.CheckMtCounty != true) {
  ...
  onSelectProvince(event : any, id : any) {
    console.log(event);
    this.model_action_change(id);
  }

  check_action_type(id : any) {
    this.img_list.forEach((e) => {
      if (e.id == id) {
        e.action_change = 'true';
      }
  ...
  check_row(arr : any, empid : any) {
    var re;
    var data_new = arr.filter((word : any) => word.emp_id == empid);
    data_new = data_new.filter((word : any) => word.action_type != 'delete');
  ...
  check_img(arr : any, visa_id : any) {
    var dt_arr;
    var data_new = arr.filter((word : any) => word.id_level_1 == visa_id);

    data_new = data_new.filter((word : any) => word.actionname == 'visa_page');
    dt_arr = data_new.filter((word : any) => word.action_type != 'delete');
    return dt_arr;
  }

  CheckAdd_visa(id : any) {
  ...
  async Config_Email_Send(emp_id : any) {
    this.modalRef?.hide();
  ...
  Send_Email() {
    ...
    const onSuccess = (data : any) => {
      ...
      console.log(data);
      if (data.after_trip.opt1 == 'true') {
        this.alert.swal_success('Send E-mail successfully.');

        data.emp_list.forEach((element : any) => {
  ...
  get Countryindoc(): Array<string> {
    const groupCountry: string[] = this.countryInDoc.map(({country_id} : any) => country_id);
    return groupCountry;
  }
  openModalz(template: TemplateRef<any>, id_detail : any) {
    ...
    const callback = (item : any) => groupCountry.includes(item.id);
  ...
  sendMailMaster() {
    this.alert.swal_success('');
  }

  checkboxAll(val : any, emp : any) {
  ...
  showitem(arr: any, code : any, empid : any) {
    var data_new = arr.filter((word : any) => word.emp_id == empid);
    data_new = data_new.filter((word : any) => word.actionname != 'visa_page');
    return data_new.length;
  }

  test_submit(check_id : any) {
    var contryX = this.country_list;
    function filter_nationality(valueX : any, typeC : any) {
      ////debugger;
      var visa;
    ...
    if (SaveState) {
      this.alert.swal_confirm_changes('do you want to submit').then((val) => {
        if (val.isConfirmed) {
  ...
  getCountryBYemp(country_city : any) {
    let obj = [];
    let ikey = 0;
  ...
  checkCountryAttach(countryList : any): boolean {
    let countryChecked = countryList.filter(({active} : any) => active);
    let statusbtn = true;
  ...
  async downloadEmployee_File(bodyX : any) {
  ...
  async downloadEmployee_Path(bodyX : any) {
    return new Promise((reslove, reject) => {
      const onSuccess = (data : any) => {
        console.log('***Call Asmx***');
        var parsed = $.parseJSON(data.d);
  ...
  async EmployeeLetter(list_country : any) {
