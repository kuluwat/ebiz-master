...
import { getBoolean, useAuth, CloneDeep } from '../../../function/globalfunction.component';
import { formatDateTime } from '../../../function/globalfunction.component';
...
export class AllowanceComponent implements OnInit {
  ...
  functionLoadData() {
    const function1Oversea = (data : any) => {
      // คำสั่งโน่นนี่ของ oversea
    };

    const function2Local = (data : any) => {
  ...
  get UserDetail() {
    return this.emp_list.filter((item : any) => item.emp_id === this.emp_idS);
  }
  get docStatus() {
  ...
  LoadKhcode() {
    ...
    const onSuccess = (data : any): void => {
      ...
      console.log(data);
      data.khcode_list.forEach((e : any) => {
        if (e.emp_id == emp_idS) {
          Khcode = true;
        }
      });
  ...
  showfromid(arr : any, empidX : any) {
    try {
      var re;
      re = arr.filter((word : any) => word.emp_id == empidX);
      return re;
    } catch (err) {
      return arr;
    }
  ...
  loaddataOnselect(empId : any, if_else : any) {
    this.appMain.isLoading = true;
    var bodyX = {
      token_login: localStorage[ 'token' ],
      doc_id: this.doc_id,
    ...
    const onSuccess = (data : any): void => {
      ...
      this.allowance_detail = this.allowance_detail.filter((word : any) => word.emp_id == empidX);
      var sunxx = 0;
      for (var a = 0; a < this.allowance_detail.length; a++) {
        var numberx = 0;
        // var xxs = this.allowance_detail[ a ].allowance_total;
      ...
      this.sumx = sunxx;
      this.allowance_detail.forEach(function (e : any) {
        if (typeof e === 'object') {
  ...
  addmaster() {
    ...
    // this.masterSetArr[ 'allowance_type' ].forEach((element : any) => {
  ...
  model_action_change_date(id : any) {
    this.model_action_change(id);
  }

  onFilterChange(txt : any) {
    let dt;
    if (txt.length >= 3) {
      if (txt == '') {
  ...
  excelclick = false;
  openModalx2Test(template: TemplateRef<any>, arr : any) {
    this.tempx = template;
    this.temparr = arr;
    this.excelclick = true;
    this.swl.toastr_success('Preview Excel');
  ...
  tempx : any;
  temparr : any;
  openModalx2(template: TemplateRef<any>, arr : any) {
    // console.log(this.data_ExcelA[ 'dailyallowance' ]);
    var i = 0;
    var allowance_detailX = this.allowance_detail;
    // this.data_ExcelA[ 'dailyallowance' ].forEach((e : any) => {
  ...
  functionRow() {
    var count = 0;
    this.email_send.forEach(function (e : any) {
  ...
  saveaction() {
    ...
    const onSuccess = (data : any): void => {
      ...
      if (this.excelclick == true) {
        ...
        this.swl.swal_success('Successfully saved');
      }
  ...
  allowance_detailF(index : any, values: any) {
    var re = this.allowance_detail;
    if (index != '') {
      // var xx  = (this.allowance_detail[ index ].new_column = values);
      console.log(this.allowance_detail);
    }
    return re;
  }

  allowance_sumFunction(low : any, midle : any, height : any, unit : any, id : any) {
    if (low == '' || low == null) {
      low = 0;
    }
    if (midle == '' || midle == null) {
    ...
    this.allowance_detail.forEach((e: any) => {
      if (e.id == id) {
  ...
  datePlusday(values : any, index : any) {
    var montF = new DatePipe('en-US');
  ...
  cancleEx() {
    this.swl.swal_confirm('Do you want to cancel the document ?', '', 'question').then((val) => {
  ...
  CancelClick() {
    ...
    console.log(xxx);
    const aa = xxx.filter((word : any) => word.emp_id == this.emp_idS);
    console.log(aa);
    var sunxx = 0;
    for (var a = 0; a < this.allowance_detail.length; a++) {
      var numberx = 0;
  ...
  total_raw : any;
  sumfunction(id : any, emp : any) {
    var sum = 0;
    this.allowance_detail.filter((e) => { });
    var re = this.allowance_detail.filter((word : any) => word.emp_id == emp);
    re = re.filter((word : any) => word.allowance_date != '');

    if (re.length <= 0) {
      return '';
    }

    re.forEach(function (e : any) {
    ...
    this.allowance_main.forEach((e : any) => {
      if (e.id == id) {
        e.grand_total = sum;
        e.action_change = 'true';
      }
  ...
  model_action_change_and_check(id : any, str_name : any) {
    //alert();

    var TTFF = 0;
    var low : any, mid : any, highV : any;
    this.allowance_detail.forEach(function (e : any) {
      if (e.id == id) {
  ...
  model_action_change(id : any) {
    ...
    this.allowance_main.forEach(function (e : any) {
      if (e.emp_id == empX) {
        e.action_change = true;
      }
    });
  ...
  sendmail() {
    ...
    const onSuccess = (data : any): void => {
      console.log(data);
      this.swl.swal_success('Send E-mail successfully.');
    };
  ...
  add_email2(event : any) {
    ...
    this.Email_sendX.forEach(function (e : any) {
      if (event == e.mail_to) {
  ...
  add_email(event : any) {
    ...
    var check_mail_dupicate = this.Email_sendX.some(
      (res : any) => res.emp_id == emp_id && res.doc_id == doc_id && event == res.mail_to && res.action_type != 'delete'
    );
    if (check_mail_dupicate) {
      this.swl.swal_error('Mail Duplicate');
    } else {
  ...
  search_len_id(ds : any, emp_id : any, doc_id : any) {
    var dt = ds.filter((res : any) => res.emp_id == emp_id && res.doc_id == doc_id);
    console.log(dt);
  ...
  export() {
    ...
    const onSuccess = (data : any): void => {
      console.log(data);
      this.swl.swal_success('Export data successfully.');
    };
  ...
  exportX(Action_param : any) {
    this.appMain.isLoading = true;
    this.generateExcel(Action_param);
  }

  commaFunction(val : any) {
    // debugger;
    try {
      if (val == 0 && val == '0') {
        return 0;
  ...
  async generateExcel(Action_param : any) {
    ...
    function add_bo_al(values : any, f1 : any, f2 : any, f3 : any, f4 : any, f5 : any) {
      // color: { argb: '595959' }
      var colorx;
      if (f5 != 0) {
        colorx = '595959';
    ...
    dataa = dataa.filter((word : any) => word.emp_id == this.emp_idS);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      let emp_id = this.emp_idS;
      var today_t = formatDateTime(today)
      ...
      if (Action_param == 'download') {
        ...
        this.swl.toastr_success('Download File');
        this.appMain.isLoading = false;
      } else {
  ...
  CheckTypeSaved(): boolean {
    ...
    if (this.arrX.allowance_main) {
      const isSaved = this.arrX.allowance_main.filter(
        ({emp_id, action_type} : any) => emp_id === this.emp_idS && action_type === 'insert'
      );
  ...
  async Config_Email_Send(i : any, input: {fInput: HTMLInputElement; fInputcc: HTMLInputElement}) {
    ...
    if (bcheck_mailtoo) {
      ...
      var irow = this.mail_list.findIndex((m_val : any) => m_val.emp_id == this.emp_idS);
      if (irow > -1) {
        // var irow = 0;
  ...
  Send_Email() {
    ...
    if (BodyX) {
      BodyX.allowance_main.forEach((e : any) => {
        if (e.emp_id == empid_x) {
          e.file_travel_report = path2x;
          e.file_report = path1x;
        }
      });
      BodyX.emp_list.forEach((e : any) => {
        if (e.emp_id == empid_x) {
          e.mail_status = 'true';
        } else {
          e.mail_status = 'false';
  ...
  countx_row(arr : any, empid : any) {
    arr.filter((word : any) => word.emp_id == empid);
    var cr_return = false;
    if (arr.length <= 0) {
      cr_return = true;
    } else {
  ...
  filter_location(id : any) {
    ...
    if (id == '' || id == null) {
      ...
      re = this.airticket_country_arr.filter((e : any) => e.id == id);
      re = re[ 0 ].city_name;
    }
  ...
  convertInt2old(Fidle: string, item? : any, id? : any, ev?: any) {
    try {
      ev = ev.toString();
    } catch (ex) { }
    var newvalue = ev.replace(/,/g, '');
  ...
  convertInt2(Fidle: string, item? : any, id? : any, ev?: any, input?: any) {
    try {
      ev = ev.toString();
    } catch (ex) { }
    var newvalue = ev.replace(/,/g, '');
...
function datediff(end : any, str : any) {
