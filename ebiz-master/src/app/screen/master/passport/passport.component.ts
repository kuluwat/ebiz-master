...
export class PassportComponent implements OnInit {
  ...
  frmHandle_Edit = {};
  @ViewChild('btntopPage', {static: true}) btntopPage?: ElementRef;
  ...
  openModal(template: TemplateRef<any>, type? : any, dr? : any, size = 'lg') {
    ...
    if (type == 'new_passport') {
      ...
      let dsemp : any = this.model_all.passport_detail.filter((res : any) => {
  ...
  uploadFile(ev : any, passport_detail_id : any, type? : any) {
  ...
  update_userByDOC(VALUE : any, select : any) {
    ...
    var status = this.model_all.passport_detail.filter((res : any) => {
      return this.emp_id == res.emp_id && res.action_change == 'true';
    });
    if (status.length > 0) {
      this.alerts.swal_confirm_changes('').then((val) => {
        ...
        if (val.isConfirmed == true) {
          ...
          this.model_all.passport_detail.forEach((res : any) => {
  ...
  updatedata(index : any, ev : any, fidle : any) {
    var emp_id = this.emp_id;
    var dt = this.model_all.passport_detail;
    var ikey = dt.findIndex((v : any) => {
  ...
  updatedatafrmhandle(index : any, ev : any, fidle : any) {
    var emp_id = this.emp_id;
    var dt = this.model_all.passport_detail;
    var ikey = dt.findIndex((v : any) => {
  ...
  def_detail() {
    this.model_all.emp_list.forEach((el : any) => {
      var search = this.model_all.passport_detail.findIndex((v : any) => {
        return v.emp_id == el.emp_id;
      });

      if (search < 0) {
        var empby_ds = this.model_all.passport_detail;
        var empby_id = empby_ds.filter((res : any) => {
  ...
  get_data_by_emp_user(ds : any) {
    if (ds.length < 1) {
      return [];
    }
    var dt = ds.filter((res : any) => {
      return res.emp_id == this.emp_id;
    });
    if (dt.length > 0) {
  ...
  get_data_by_emp(ds : any) {
    ...
    if (dt.length > 1) {
      dt = dt.sort((a : any, b : any) => a.sort_by - b.sort_by);
      return dt;
    } else {
      if (dt.length == 1) {
        dt = dt.sort((a : any, b : any) => a.sort_by - b.sort_by);
  ...
  get_data_by_passport_new() {
    let dsemp : any = this.model_all.emp_list.filter((res : any) => {
      return res.emp_id == this.emp_id;
    })[ 0 ];
  ...
  get_data_passport_by_id(ds : any, id : any) {
    let ikey = ds.findIndex((res : any) => {
  ...
  def_data(data : any, oldData? : any) {
    ...
    if (oldData) {
      for (let detail of data.passport_detail) {
        ...
        const findPanel = oldData.find(({id, emp_id} : any) => detailID == id && emp_id === detailemp);
        if (findPanel) {
  ...
  setActive(ds : any) {
    ...
    const checkCountRow = passport_detail.some((item : any) => item.default_type === 'true');
    const findActive = passport_detail.filter((item : any) => item.default_type === 'true');
    const OtherActive = passport_detail.filter((item : any) => item.default_type !== 'true');
    if (checkCountRow) {
      ds.passport_detail = [ ...findActive, ...OtherActive ];
      ds.passport_detail.forEach((item : any, i : any) => {
        item.sort_by = i + 1;
      });
      ds.passport_detail.sort((a : any, b : any) => a.sort_by - b.sort_by);
    }
  }
  add_column(ds : any) {
    ds.forEach((el : any) => {
      ...
      if (el[ 'passport_date_issue' ] == null || el[ 'passport_date_issue' ] == '') {
        ...
        el[ 'passport_date_issue_type' ] = this.convert_dateDMY(el[ 'passport_date_issue' ], 'yyyy-MM-dd');
      }
      if (el[ 'passport_date_expire' ] == null || el[ 'passport_date_expire' ] == '') {
        el[ 'passport_date_expire_type' ] = null;
      } else {
        el[ 'passport_date_expire_type' ] = this.convert_dateDMY(el[ 'passport_date_expire' ], 'yyyy-MM-dd');
      }
      if (el[ 'passport_date_birth' ] == null || el[ 'passport_date_birth' ] == '') {
        el[ 'passport_date_birth_type' ] = null;
      } else {
        el[ 'passport_date_birth_type' ] = this.convert_dateDMY(el[ 'passport_date_birth' ], 'yyyy-MM-dd');
      }
  ...
  Change_Passport(dr : any) : boolean | void {
  ...
  Add_Passport(dr: any): boolean | void {
    debugger;
    ...
    if (this.frmValidator(dr, 'addpassport') == false) {
      ...
      if (dt.length == 0) {
        return; // Return void if no passport details
      }
      ...
      if (check_no_passport) {
        return; // Return void if passport number check fails
      }
  
      var doc_id = this.doc_id;
      var emp_id = this.emp_id;
      var last_id = dt
        .map((mx: any) => parseInt(mx.id))
        .sort((a, b) => {
          return a - b;
        });
      ...
      var xrowTrue = 1;
      for (let j in dt) {
        // if (dt[j].default_type == 'true') {
        //   dt[j].sort_by = xrowTrue.toString();
        // } else {
        //   dt[j].sort_by = xrow.toString();
        // }
        xrow++;
      }
      ...
      this.frmHandle = {...this.afterNewpassport};
      this.alerts.toastr_success('Add Passport');
      this.hide_modal('new_passport', '');
  
      return; // Ensure the function returns after successful operation
    }
  }
  

  Add_ImgDeatil(drimg : any, passport_detail_id : any, type? : any) {
    if (drimg.length > 0) {
      if (type == 'edit') {
        try {
          this.model_all.img_list.forEach((res) => {
  ...
  CheckImg_Edit(drimg : any, passport_detail_id : any) {
    this.model_all.img_list.forEach((res) => {
      if (
        res.emp_id == this.emp_id &&
        res[ 'id_level_1' ] == passport_detail_id &&
  ...
  Check_Passport_number(dr : any, type? : any): boolean {
  ...
  Add_Passport_NULL_DATA(dr : any) {
    if (true) {
      ...
      this.frmHandle = {...this.afterNewpassport};
      // this.topFunction();
      this.alerts.toastr_success('New Passport');
    }
  }

  // function แบบ กด active แล้วขึ้นไปเป็นอันแรก
  Sort_by_passport2(ds : any, dr : any) {
    let i_index = ds.findIndex((res : any) => res.id == dr.id);
    let old_data = [ ds[ i_index ] ];
    let new_data = ds.filter((val : any, i : any) => i != i_index);
    let emp_id = this.emp_id;
    if (new_data.length > 0) {
      new_data.forEach((el : any) => {
        old_data.push(el);
      });
      let irow = 1;
  ...
  Sort_by_passport(ds : any, dr : any) {
    let old_data = ds;
    let emp_id = this.emp_id;
    if (old_data.length > 0) {
      // let irow = 1;
      old_data.forEach((res : any) => {
        if (emp_id == res.emp_id) {
          if (res.id == dr.id) {
            if (res.default_type_bool == 'false' || res.default_type_bool == false) {
              res.default_type = 'false';
  ...
  frmValidator(dr : any, action_stage : any) {
    const validator = function (dr : any): boolean {
      var bcheck = false;
  ...
  Onsave(btn_type: any, tep?: any, dr?: any): void {
    debugger;
    console.log('>> passport_detail');
    console.log(this.model_all.passport_detail);
  
    if (!this.validator_check) {
      this.Onsave('saved', '', '');
      this.vaildator_save = true;
      // this.appMain.showMessage('Need to input value * ');
      return;
    ...
    if (btn_type === 'save') {
      let accept_true = false;
      if (dr && dr.accept_type) {
        accept_true = dr.accept_type;
      }
  
      if (accept_true) {
        this.alerts.swal_confirm_changes('Do you want to save the data?').then((val) => {
          if (val.isConfirmed) {
      ...
      let bcheck = tep !== 'modal';
  
      if (bcheck) {
        this.Appmain.isLoading = true;
        let bodyX = this.model_all;
        bodyX.passport_detail.forEach((e: any) => {
          e.default_action_change = true;
        });
  
        let Panel_List = bodyX.passport_detail.map(({ panel, id, emp_id }: any) => ({ id, panel, emp_id }));
        ...
        const OnsaveSucecss = (data: any) => {
          ...
          if (data.after_trip.opt1 === 'true') {
            ...
            let index = this.model_all.passport_detail.findIndex((res: any) => res.emp_id === this.emp_id);
  ...
  OnsaveAll(btn_type: any): boolean | void {
    ...
    if (this.validator_check == false) {
      this.vaildator_save = true;
      // this.appMain.showMessage('Need to input value * ');
      return true; // Return true to indicate an issue with validation
    } else {
      if (btn_type == 'saved') {
        var bcheck = true;
        if (bcheck) {
          ...
          return; // Ensure the function returns after async operation
        }
      }
    }
  }
  ...
  OnsaveUpload() {
    ...
    if (bodyX.passport_detail) {
      Panel_List = bodyX.passport_detail.map(({panel, id, emp_id} : any) => ({id, panel, emp_id}));
    }
    const OnsaveSucecss = (data : any) => {
      this.typeLoad = false;
      ...
      if (data.after_trip.opt1 == 'true') {
        ...
        // this.setActive(data);
        this.def_data(data, Panel_List);
        let index = this.model_all.passport_detail.findIndex((res :any) => {
          return res.emp_id === this.emp_id;
        });
  ...
  check_action(item : any, type: any) {
    // console.log(item)
    var bcheck = false;
  ...
  CheckACTION(): boolean {
    ...
    if (data_def.length > 0) {
      bcheck = data_def.some((res : any) => res.emp_id == emp_id && res.action_change == 'true');
    }
    return bcheck;
  }
}
