...
export class TravelinsuranceComponent implements OnInit {
  ...
  get UserDetail() {
    return this.emplist.filter((item) => item.emp_id === this.empid);
  }
  get docStatus() {
    return (Status: number) => {
      ...
      if (this.emplist.length > 0) {
        ...
        let dt = this.emplist.find((item) => item.emp_id === emp_id);
        if (dt) {
          // alert(1);
  ...
  load_broker() {
    ...
    const onSuccess = (data): void => {
      this.brokerDt = data.master_insurancebroker.filter((word) => word.status == '1');
    };
  ...
  convertInt(Fidle: string, item?, ev?: any) {
    try {
      ev = ev.toString();
    } catch (ex) { }
  ...
  convertInt2(Fidle: string, item?, ev?: any, id?) {
    try {
      ev = ev.toString();
    } catch (ex) { }
    var newvalue = ev.replace(/,/g, '');
  ...
  map_emp() {
    this.img_list.forEach((e) => {
      this.travelInsurance_detail.forEach((x) => {
  ...
  onloadX() {
    ...
    const onSuccess = (data): void => {
      ...
      if (this.brokerDt.length > 0) {
        dtborker = this.brokerDt.reduce((acc, cur) => {
      ...
      var panelArr = [];
      let scope = this;
      this.travelInsurance_detail.forEach(function (e) {
        if (typeof e === 'object') {
      ...
      if (data.user_admin === false) {
        ...
        let finduser = data.emp_list.find(({emp_id}) => emp_id === this.empid);
        if (finduser) {
        ...
        const {userDisplay = null} = data.emp_list.find(({emp_id: emp_idx}) => emp_idx === emp_id);
        if (userDisplay) {
  ...
  data_retune(emp_id) {
    var datrf = this.travelInsurance_detail.filter((word) => word.emp_id == emp_id);
    var re = datrf.filter((word) => word.action_type != 'delete');
    var NewDataSort = re.sort(function (a, b) {
  ...
  data_retune_length(id, emp_id) {
    var datrf = this.travelInsurance_detail.filter((word) => word.emp_id == emp_id);
    var re = datrf.filter((word) => word.action_type != 'delete');
    var DataSet = this.travelInsurance_detail.filter((word) => word.id == id);
  ...
  addrow(emp_id) {
    ...
    debugger;
    var DataSet = this.travelInsurance_detail.filter((word) => word.emp_id == emp_id);
    ...
    console.log('Addrow -------------');
    console.log(this.travelInsurance_detail);
    this.swl.toastr_sucess('Add Insurance Plan');
  ...
  XorderBy() {
    ...
    var NewDataSort = this.travelInsurance_detail.sort(function (a, b) {
    ...
    var NewDatafilter = this.travelInsurance_detail.filter((word) => word.emp_id == empidX);
  ...
  duration(values1, values2, index) {
  ...
  async Cancel_editClick() {
    const actioncancel = await this.swl.swal_confrim_changes('Do you want to cancel the document ?');
  ...
  datePlusday3(values, index, text) {
    var montF = new DatePipe('en-US');
  ...
  Config_Email_Send(i) {
    ...
    if (bcheck_mailtoo) {
      ...
      var irow = this.mail_list.findIndex((m_val) => m_val.emp_id == this.emp_id);
      if (irow > -1) {
  ...
  path_file_name;
  path_file_path;
  testCallAsmx(id, typex) {
    this.appMain.isLoading = true;
    //ถ้าไม่มีข้อมูลให้ใส่ขีด - แทนค่าว่างก่อนส่งไป
    var stebody = this.travelInsurance_detail.filter((word) => word.id == id);
  ...
  check_values_beforeSend(id) {
    var SaveState = true;
    var StatusSave = [],
      indexx = 0;
    var result = this.travelInsurance_detail.filter((word) => word.id == id);
    result.forEach((e) => {
      var TextReWaring = '';
  ...
  check_values_beforeSendmail(id, btnType) {
    var SaveState = true;
    var StatusSave = [],
      indexx = 0;
    var result = this.travelInsurance_detail.filter((word) => word.id == id);
