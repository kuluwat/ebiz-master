...
export class MtfeedbackquestionComponent implements OnInit {
  ...
  get_one_data(type: any): Observable<any> {
    if (type === "master_type") {
        ...
        const feedback_type = this.ws.callWs(BodyX, this.action_stage.master);
    ...
    if (type === "master_topic") {
        ...
        const feedback_list = this.ws.callWs(BodyXx, this.action_stage.master);
    ...
    if (type === "master_question") {
        ...
        const feedback_detail = this.ws.callWs(BodyXxx, this.action_stage.master);
        return forkJoin([feedback_detail]);
    }
    // Default return statement for undefined cases
    return of([]);
}
  updata_for_api(type : any) {
    ...
    await this.get_data_master().subscribe(
      ...
      () => {
        ...
        if (this.model_all.list_feedback.length > 0) {
          ...
          this.model_all.list_feedback.forEach((el : any) => {
            el["show"] = true;
          });
        }
  ...
  swal_confrim_main(
    ...
    if(type_action == "delete"){
      ...
      else{   return this.alerts.swal_confirm_changes("");}
  ...
  swal_confirm(type_data = "data", type_action = "delete") {
    var txt = "";
    if (type_action == "delete") {
      txt = "Do you want to " + type_action + " ?";
    } else {
  ...
  //question
  new_row(rowquestion : any, rowtype : any) {
    var dt = [];
  ...
  async tabChanged(previous_type : any, tabRef : any, curindex : any) {
    ...
    if (curindex == 0) {
      ...
      //   (res : any) => res.action_change == "true"
      // );

      // if (check_action_type) {
      //   await this.swal_confirm("data", "save").then((res) => {
  ...
  async update_mat_tab(for_of : any, tabRef : any) {
    // 0 = Feedback Type // no check
    // 1 = Feedback Topic  // check type
    // 2 = Feedback Questions" // check type and check topic
    // check_val
  ...
  selected_type_now(row_type : any, tem : any) {
    this.Feedback_type_selected = {
      name: row_type.name,
  ...
  change_status(type_checked : any, item : any) {
    this.swal_confirm("change", "save").then((val) => {
  ...
  //update selected_type_now
  selected_topic_now(row_type : any, tem : any) {
    this.Feedback_topic_selected = {
      name: row_type.name,
      status: row_type.status,
      id: row_type,
  ...
  //#endregion
  get_index_question(ds: any, row_type: any, row_question: any) {
    if (ds.length > 0) {
  ...
  update_data(item: any, updae_action : any, row_type? : any) {
    this.swal_confirm().then((val) => {
      if (val.isConfirmed == true) {
        item.action_type = "delete";
        item.action_change = "true";
        var num_row_delete = this.model_all.detail_feedback.filter((res: any) => {
          return (
  ...
  Onsave_type(btn_type : any, type? : any) {
    if (btn_type == "saved") {
      if (true) {
        this.Appmain.isLoading = true;
  ...
  Onsave_topic(btn_type : any, type? : any) {
    if (btn_type == "saved") {
      if (true) {
        this.Appmain.isLoading = true;
        const OnsaveSucecss = (data : any) => {
  ...
  DataBYTYPEID(ds: any, topic_id: any, type?: any) {
    ...
    if (type == "master") {
      ...
      dt = ds.filter((res: any) => res.action_type != "delete");
    } else {
      dt = ds.filter((res: any) => res.id_main == feedback_type_id_checked);
    }
    return dt;
  }
