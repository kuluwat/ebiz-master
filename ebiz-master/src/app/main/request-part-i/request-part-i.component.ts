...
export class RequestPartIComponent implements OnInit {
  ...
  didFetchData() {
    ...
    const onSuccess = (dao : any) => {
      ...
      var bucketProvince = []
      dao[ "province" ].forEach((current : any) => {
        bucketProvince.push({
      ...
      var bucketCountry = []
      dao[ "country" ].forEach((current : any) => {
        bucketCountry.push({
  ...
  onSelectCountryAddTravel(event : any) {
    //alert('xx')
    console.log('-----------------------------------------')
    ...
    if (this.model.add_travel.country.value != undefined) {
      if (this.type == "oversea" || this.type == "overseatraining") {
        ...
        // this.model.add_travel.business_date = this.model.business_date != null || 
        // // this.model.business_date != [] ? this.model.business_date : [];
  ...
  updateTravelDate(path: String) {
    ...
    if (path == "root") {
      if (
        // this.model.travel_date != [] || 
        this.model.travel_date != undefined || this.model.travel_date != null) {
      ...
      if (
        // this.model.add_travel.travel_date != [] ||
         this.model.add_travel.travel_date != undefined || this.model.add_travel.travel_date != null) {
        // this.calculateDurationDay(
  ...
  didConvertFormatValue(action: String, remarks: String): any {
    ...
    var data = {
      ...
      // "id_company": 
      // this.model.company.value === [] ||
      //  this.model.company.value == undefined ? "" : this.model.company.value[ "index" ],
      // "type_of_travel": {
  ...
  handleActionPages(action: stateAction) {
    ...
    const submit = () => {
      ...
      if (requestData()) {
        ...
        if (this.model.summary_table.length == 0 || 
          // this.model.summary_table == [] || 
          this.model.summary_table == undefined || this.model.summary_table == null) {
          this.panel.travel = true
    ...
    const reject = () => {
      ...
      // } else {
      this.appMain.showConfirmTextbox("Do you want to reject document, please input to remark", (remarks : any) => {
        if (remarks == "") {
    ...
    const revise = () => {
      ...
      //   console.log("No value");
      // } else {
      this.appMain.showConfirmTextbox("Do you want to revise document, please input to remark", (remarks : any) => {
        if (remarks == "") {
          this.appMain.showMessage("Please tell me the reason")
          return
        }
  ...
  //   }
  //   else {
  //     if (this.model.travel_date != null && 
  //       // this.model.travel_date != [] &&
  //        this.model.travel_date.length === 2) {
  //       if (type === 'min') {
